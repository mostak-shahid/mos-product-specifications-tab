...
const units = [
    { value: '', label: __('None', 'mos-product-specifications-tab') },
    { value: 'px', label: 'px' },
    { value: '%', label: '%' },
    { value: 'em', label: 'em' },
    { value: 'rem', label: 'rem' },
    ...
    return (
        <>
            <div className={`font-wrapper ${className}`}>
                <Space align='center'>
                    <Switch 
                        aria-label={__('Enable Font Options', 'mos-product-specifications-tab')}
                        checked={enableFont}
                        onChange={(enabled) => updateValue('enabled', enabled)}
                    />
                    <Typography.Title heading={6} style={{ margin: 8 }}>
                        {enableFont ? __('Enabled', 'mos-product-specifications-tab') : __('Disabled', 'mos-product-specifications-tab')}
                    </Typography.Title>
                </Space>  
                {
                    enableFont && 
                            ...
                            {options.map((option) => (
                                <div key={option} className={`mb-2 from-group from-group-${option} col-${(option === 'color' || option === 'font-size') ? '12' : '6'}`}>
                                    ...
                                    {/* font-family → text input */}
                                    {
                                        option === "color" ? (
                                            <div className='mb-2'>
                                                <ColorPickerControl
                                                    ...
                                                    label={__('Font Color', 'mos-product-specifications-tab')}
                                        ...
                                        (
                                            <UnitControl
                                                label={__('Size', 'mos-product-specifications-tab')}
                                        ...
                                        (
                                            <>
                                                ...
                                                <Select
                                                    ...
                                                    //     return {
                                                    //         label: __(valStr.charAt(0).toUpperCase() + valStr.slice(1), 'mos-product-specifications-tab'),
                                                    //         value: valStr,
                                                    //     };
                                                    // })}
                                        ...
                                        (
                                            <UnitControl
                                                label={__('Line Height', 'mos-product-specifications-tab')}
                                                onChange={(value) => updateValue(option, value)}
                                                value={values[option]}
                                                units={units}
                                                min={0.8}
