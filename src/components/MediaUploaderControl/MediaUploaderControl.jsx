...
export default function MediaUploaderControl({ data={}, name, handleChange, options={}, className='' }) {    
    ...
    const runUploader = (event) => {
        ...
        frame = wp.media({
            title: options?.frame?.title || __("Select or Upload Image", "mos-product-specifications-tab"),
            button: {
                text: options?.buttons?.select || __("Use this image", "mos-product-specifications-tab"),
            },
    ...
    return (
        <>
            {/* {console.log(data)} */}
            <div className={`mos-product-specifications-tab-media-uploader-unit ${className}`}>
                    ...
                    <Col xs={12}>
                        { media?.url && media?.id ?                     
                            <div className="file-name background-primary with-close-button">
                                ...
                                <Dashicon className="mos-product-specifications-tab-remove-image text-danger" onClick={removeImage} icon="remove" />
                            </div> : 
                            <div className="file-name background-primary d-flex align-items-center justify-content-center" onClick={runUploader}>
                                <div className="no-media-wrap">
                                    ...
                                    <div className="text-wrap">
                                        <span className="title">{__("Upload Media", "mos-product-specifications-tab")}</span>
                                        <span className="sub-title">{__("Use the upload button", "mos-product-specifications-tab")} <br/> {__("and select media  ", "mos-product-specifications-tab")}</span>
                                    </div> 
                        ...
                        <div className="file-detail">
                            <InputGroup>
                                <Button
                                    ...
                                    onClick={ runUploader }
                                >
                                    {options?.buttons?.upload || __("Upload Image", "mos-product-specifications-tab")}
                                </Button>    
                                <Button
                                    theme="solid"
                                    type="secondary"
                                    ...
                                    className='ml-2'
                                >
                                    {options?.buttons?.remove || __("Remove Image", "mos-product-specifications-tab")}
                                </Button>                                           
                            </InputGroup>    
