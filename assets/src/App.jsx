...
import { Dashboard, Settings, General, ImportExport, More, Tools, Logs, LogsCharts, LogsTable, Feedback, FreeVsPro, NotFound} from './pages';
...
export default function App() {
    ...
    const markNewsAsRead = async (newsId) => {
        if (!readNewsIds.includes(newsId)) {
            ...
            try {
                await apiFetch({
                    path: '/mos-product-specifications-tab/v1/set-option',
                    method: 'POST',
                    data: {
    ...
    return (
        <LocaleProvider locale={en_US}>
            <div className="mos-product-specifications-tab-settings-container" style={{backgroundColor: 'var(--semi-color-bg-1)'}}>
                {!mos_product_specifications_tab_ajax_obj?.isPro &&
                    <Banner 
                ...
                >
                    {newsItems.length === 0 ? (
                        <p>{__("Loading news...", "mos-product-specifications-tab")}</p>
                    ) : (
                        <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                                ...
                                {newsItems
                                    ...
                                    <Card key={item.id} style={{ marginBottom: 12, backgroundColor: !readNewsIds.includes(item.id) ? 'var(--semi-color-primary-light-default)' : '' }} bodyStyle={{ padding: 10 }}>
                                        ...
                                        <div className='mt-2'>
                                            ...
                                            >
                                                {__("Read more", "mos-product-specifications-tab")}
                                            </Button>
                                        </div>
                                    </Card>
                                ))}
                            ...
                            {Math.ceil(newsItems.length / itemsPerPage) > 1 && (
                                <div style={{ borderTop: '1px solid var(--semi-color-border)', flexShrink: 0 }} className='flex justify-between items-center py-4 mt-4'>
                                    ...
                                    >
                                        {__("Previous", "mos-product-specifications-tab")}
                                    </Button>
                                    <Text>
                                        {__("Page", "mos-product-specifications-tab")} {newsCurrentPage} {__("of", "mos-product-specifications-tab")} {Math.ceil(newsItems.length / itemsPerPage)}
                                    </Text>
                                    <Button
                                        size="small"
                                        onClick={() => setNewsCurrentPage(newsCurrentPage + 1)}
                                    ...
                                    >
                                        {__("Next", "mos-product-specifications-tab")}
                                    </Button>
                                </div>
                            )}
                        </div>
