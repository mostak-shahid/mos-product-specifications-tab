...
class More
{
	...
	public function add_footer_script()
	{
		...
		if (isset($this->options['more']['css']) && !empty($this->options['more']['css'])) {
			echo '<style id="mos_product_specifications_tab_style">' . wp_kses_post($this->options['more']['css']) . '</style>';
		}
		if (isset($this->options['more']['js']) && !empty($this->options['more']['js'])) {
			echo '<script id="mos_product_specifications_tab_script">' . wp_kses_post($this->options['more']['js']) . '</script>';
		}
	}
}
